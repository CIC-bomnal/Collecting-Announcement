...
마감일 7일 이상 남은 공고를 나라장터와 K-Startup API에서 수집하여 Google Sheets에 증분 업데이트하는 시스템입니다.
GitHub Actions로 평일 매일 09:00(KST)에 자동 실행됩니다.

## 주요 기능

- **나라장터 + K-Startup** 공고 수집 (API)
- **키워드/마감일 필터링** (7일 이상 남은 공고만)
- **PDF 사업명 매칭**: 2026 창업지원사업 안내서(509개 사업)와 K-Startup 공고를 fuzzy 매칭
- **증분 업데이트**: 신규 공고는 추가, 기존 공고는 변경된 필드 전체 갱신
- **중복 제거**: 공고ID 기준 자동 중복 정리
- **금지어 필터링**: 육성기업 관점에서 불필요한 공고 제외 (별도 탭)
...
```
[나라장터]  API fetch (오늘~4개월) → 키워드 필터 → 마감일 필터 → 스프레드시트 업데이트
[K-Startup] API fetch (전체) → 마감일 필터 → (키워드 OR PDF 사업명 매칭) → 업데이트 + 하이라이팅
[PDF]       목차 파싱 → 상세페이지 날짜 추출 → "2026 창업지원사업" 탭 업로드
[금지어]    나라장터/K-Startup 결과에서 금지어 제외 → 필터 탭 업로드
```
...
육성기업 관점에서 확인 불필요한 공고를 제외합니다.

- 환경변수 `EXCLUSION_KEYWORDS`에 쉼표 구분으로 금지어 설정 (예: `지원기업 모집,창업기업 모집`)
- 원본 탭(나라장터, K-Startup)은 그대로 유지
- 금지어가 제목에 포함된 공고를 제외한 결과를 **별도 필터 탭**에 업로드
...
| `KEYWORDS` | 키워드 (쉼표 구분) |
| `EXCLUSION_KEYWORDS` | 금지어 (쉼표 구분, 선택) |

> GitHub Secrets는 보안상 저장 후 값을 다시 볼 수 없습니다 (write-only). 빈 칸으로 보여도 값이 입력되어 있으면 정상입니다.
...
### 나라장터(필터) 탭 (8열)
나라장터 탭에서 금지어 제외한 결과. 열 구성 동일.

### K-Startup(필터) 탭 (8열)
K-Startup 탭에서 금지어 제외한 결과. 열 구성 동일.
...
- 기존 공고 갱신 시 마감일·예산 등 변경된 필드 전체 갱신 (등록일자만 유지)
...
│   ├── filter.py              # 키워드/마감일/PDF매칭/금지어 필터링
...
### GitHub Secrets 값이 안 보임
→ 정상입니다. GitHub Secrets는 보안상 저장 후 다시 볼 수 없습니다. Actions 실행 로그에서 `✓ 환경변수 검증 완료`가 나오면 정상 설정된 상태입니다.
